...
class SSLPinst(master.StochIPinst):
    ...
    def Initialize(self):
        ...
        for s in range(self.Nscen):
            ...
            for i in range(self.Nn):
                for j in range(self.Nfv):
                    y[i, j] = self.scensub[s].addVar(vtype=GRB.BINARY, obj=-regCoefy * self.qMtr[i, j],
                                                     name="y_" + str(i) + "," + str(j))
            y0 = {}
            for j in range(self.Nfv):
            ...
            for j in range(self.Nfv):
                self.scensub[s].addConstr(
                    quicksum(self.dMtr[i, j] * y[i, j] for i in range(self.Nn)) - y0[j] <= self.Nu * x_scen[j])
            for i in range(self.Nn):
        ...
        for s in range(self.Nscen):
            ...
            for i in range(self.Nn):
                for j in range(self.Nfv):
                    y[i, j] = self.scenrc[s].addVar(vtype=GRB.BINARY, obj=-self.qMtr[i, j],
                                                    name="y_" + str(i) + "," + str(j))
            y0 = {}
            for j in range(self.Nfv):
    ...
    def SolveExtensive(self):
        ...
        for s in range(self.Nscen):
            for j in range(self.Nfv):
                Extensive.addConstr(
                    quicksum(self.dMtr[i, j] * y[s, i, j] for i in range(self.Nn)) - y0[s, j] <= self.Nu * x[j])
        for s in range(self.Nscen):
            for i in range(self.Nn):
        ...
        for s in range(self.Nscen):
            theta_value[s] = sum(
                -self.qMtr[i, j] * y[s, i, j].x for i in range(self.Nn) for j in range(self.Nfv)) + sum(
                self.q0Vec[j] * y0[s, j].x for j in range(self.Nfv))
        print(theta_value)
    ...
    def SolveScenSub(self, scen_id, objCoef, regCoefy, tlimit, MIPgap='nan'):
        ...
        for k in range(self.scensub[scen_id].SolCount - 1):
            ...
            for j in range(self.Nfv):
                soln[j] = modelSoln[j]
            soln[self.Nfv] = -sum(
                self.qMtr[i, j] * modelSoln[self.Nfv + i * self.Nfv + j] for i in range(self.Nn) for j in
                range(self.Nfv)) + \
            ...
            if where == GRB.Callback.MIPSOL:
                ...
                for s in range(self.Nscen):
                    ...
                    if theta_value[s] < ObjV - tol * (abs(theta_value[s]) + 1):
                        model.cbLazy(self.theta[s] >= ObjV - (ObjV - ScenLB[s]) * (
                                    quicksum((1 - self.x[j]) for j in J1) + quicksum(self.x[j] for j in J0)))
