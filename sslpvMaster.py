...
class SSLPVinst(master.StochIPinst):
    ...
    def Initialize(self):
        ...
        for s in range(self.Nscen):
            ...
            for i in range(self.Nn):
                for j in range(self.m):
                    ...
                    y[i, j] = self.scensub[s].addVar(lb=0, ub=1, obj=-regCoefy * self.qMtr[i, j],
                                                     name="y_" + str(i) + "," + str(j))
            y0 = {}
            for j in range(self.m):
            ...
            for j in range(self.m):
                ...
                self.scensub[s].addConstr(
                    quicksum(self.dMtr[i, j] * y[i, j] for i in range(self.Nn)) - y0[j] <= self.Nu * (1 - self.rho) *
                    x_scen[j] + self.Nu * self.rho * x_scen[j + self.m])
            for i in range(self.Nn):
        ...
        for s in range(self.Nscen):
            ...
            for j in range(self.m):
                self.scenrc[s].addConstr(quicksum(self.dMtr[i, j] * y[i, j] for i in range(self.Nn)) - y0[j] <= 0.0,
                                         name="xconstr" + str(j))
            for i in range(self.Nn):
    ...
    def SolveExtensive(self):
        ...
        for s in range(self.Nscen):
            for j in range(self.m):
                Extensive.addConstr(quicksum(self.dMtr[i, j] * y[s, i, j] for i in range(self.Nn)) - y0[s, j] <= (
                            1 - self.rho) * self.Nu * x[j] + self.rho * self.Nu * x_con[j])
        for s in range(self.Nscen):
            for i in range(self.Nn):
    ...
    def BuildPrimal(self):
        ...
        self.PrimalMaster.setObjective(quicksum(self.pVec[s] * self.theta[s] for s in range(self.Nscen)) + quicksum(
    ...
    def readData(self):
        ...
        self.Nscen = fline[2]
        # Read cVec
        self.cVec = np.append((1 - self.rho) * self._readVec(x[1], np.float32),
                              self.rho * self._readVec(x[1], np.float32))
        # Read qMtr
    ...
    def SolveBendersSub(self, scen_id, x_input):
        ...
        for j in range(self.m):
            benderssub.getConstrByName("xconstr" + str(j)).RHS = self.Nu * (
                        (1 - self.rho) * x_input[j] + self.rho * x_input[j + self.m])
        benderssub.update()
        benderssub.optimize()
    ...
    def SolveScenSub(self, scen_id, objCoef, regCoefy, tlimit, MIPgap='nan'):
        ...
        yObjV = -sum(
            self.qMtr[i, j] * self.scensub[scen_id].getVarByName("y_" + str(i) + "," + str(j)).x for i in range(self.Nn)
            for j in range(self.m)) + \
        ...
        for k in range(self.scensub[scen_id].SolCount - 1):
            ...
            for j in range(self.Nfv):
                soln[j] = modelSoln[j]
            soln[self.Nfv] = -sum(
                self.qMtr[i, j] * modelSoln[self.Nfv + i * self.m + j] for i in range(self.Nn) for j in range(self.m)) + \
    ...
    def SolveRecourse(self, scen_id, x_input):
        for j in range(self.m):
            self.scenrc[scen_id].getConstrByName("xconstr" + str(j)).RHS = self.Nu * (1 - self.rho) * x_input[
                j] + self.Nu * self.rho * x_input[self.m + j]
    ...
    def BendersBnC(self, timeLimit=60 * 60, tol=1e-6, cutTime=0, methodName='BnC'):
            ...
            if where == GRB.Callback.MIPSOL:
                ...
                for s in range(self.Nscen):
                    ...
                    if theta_value[s] < ObjV - tol * (abs(theta_value[s]) + 1):
                        ...
                        model.cbLazy(model.getVarByName("theta" + str(s)) >= const + quicksum(
                            subg[i] * model.getVarByName("x" + str(i)) for i in range(self.Nfv)))
        ...
        wrtStr = "ObjVal: " + str(self.PrimalMaster.objval) + "\tObjBnd: " + str(
            self.PrimalMaster.ObjBound) + "\tBnCTime: " + str(time.time() - t0) + "\tNo. of MIP solns: " + str(
            MIPSOLcount) + "\tBendersTime: " + str(tBenders) + "\tRecourseTime: " + str(
            tRecourse) + "\tTotalTime: " + str(cutTime + time.time() - t0) + "\n"
